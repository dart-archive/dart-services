...
void defineTests() {
  group('Flutter SDK analysis_server', () {
    ...
    test('analyze counter app', () async {
      final results =
          await analysisServer.analyze(sampleCodeFlutterCounterNullSafe);
      expect(results.issues, isEmpty);
    });

    test('analyze Draggable Physics sample', () async {
      final results =
          await analysisServer.analyze(sampleCodeFlutterDraggableCardNullSafe);
      expect(results.issues, isEmpty);
    });
  });
  ...
  group('Flutter SDK analysis_server with analysis servers', () {
    ...
    test('analyze counter app', () async {
      final results = await analysisServersWrapper
          .analyze(sampleCodeFlutterCounterNullSafe);
      expect(results.issues, isEmpty);
    });

    test('analyze Draggable Physics sample', () async {
      final results = await analysisServersWrapper
          .analyze(sampleCodeFlutterDraggableCardNullSafe);
      expect(results.issues, isEmpty);
    });
  });
  ...
  group('CommonServerImpl flutter analyze', () {
    ...
    test('counter app', () async {
      final results = await commonServerImpl
          .analyze(SourceRequest()..source = sampleCodeFlutterCounterNullSafe);
      expect(results.issues, isEmpty);
    });

    test('Draggable Physics sample', () async {
      final results = await commonServerImpl.analyze(
          SourceRequest()..source = sampleCodeFlutterDraggableCardNullSafe);
      expect(results.issues, isEmpty);
    });
  });
}

...
void defineTests() {
  ...
  Future<MockHttpResponse> _sendPostRequest(
    ...
    final uri = Uri.parse('$PROTO_API_URL_PREFIX/$path');
  ...
  group('CommonServerProto JSON', () {
    setUpAll(() async {
      ...
      // happening and deal with that in warmup/init.
      {
        ...
        while (decodedJson.isEmpty) {
          final response =
              await _sendPostRequest('analyze', jsonData);
    ...
    test('compile', () async {
      final jsonData = {'source': sampleCode};
      final response =
          await _sendPostRequest('compile', jsonData);
      expect(response.statusCode, 200);
      final data = await response.transform(utf8.decoder).join();
    ...
    test('compileDDC', () async {
      final jsonData = {'source': sampleCode};
      final response =
          await _sendPostRequest('compileDDC', jsonData);
      expect(response.statusCode, 200);
      final data = await response.transform(utf8.decoder).join();
      expect(json.decode(data), isNotEmpty);
    });
    ...
    test('complete', () async {
      ...
      final response =
          await _sendPostRequest('complete', jsonData);
      expect(response.statusCode, 200);
      final data = json.decode(await response.transform(utf8.decoder).join());
      expect(data, isNotEmpty);
    });
    ...
    test('complete no data', () async {
      final response = await _sendPostRequest('complete', {});
      expect(response.statusCode, 400);
    });
    ...
    test('document', () async {
      ...
      final response =
          await _sendPostRequest('document', jsonData);
      expect(response.statusCode, 200);
      final data = json.decode(await response.transform(utf8.decoder).join());
      expect(data, isNotEmpty);
    });
    ...
    test('format', () async {
      ...
      final response =
          await _sendPostRequest('format', jsonData);
      expect(response.statusCode, 200);
      final data = json.decode(await response.transform(utf8.decoder).join());
    ...
    test('fix', () async {
      ...
      final response =
          await _sendPostRequest('fixes', jsonData);
      expect(response.statusCode, 200);
      final data = json.decode(await response.transform(utf8.decoder).join());
    ...
    test('assist', () async {
      ...
      final response =
          await _sendPostRequest('assists', jsonData);
      expect(response.statusCode, 200);
    ...
    test('version', () async {
      final response = await _sendGetRequest('version');
      expect(response.statusCode, 200);
      final data = json.decode(await response.transform(utf8.decoder).join());
